...
describe("Pump", function () {
    ...
    async function getTradeSignature(user) {
        const dataHash = ethers.solidityPackedKeccak256(['uint256', 'address'],
            [97, user]
        )
    ...
    describe("Before list", function () {
        ...
        it("Cannt buy token by invalid signature", async () => {
            ...
            const signature = await getTradeSignature(bob.address)
            await expect(token.connect(alice).buyToken(buyAmount, ethers.ZeroAddress, 0, signature, {
                value: buyFund
            })).to.revertedWithCustomError(token, 'InvalidGatePermission')
        })
        ...
        it("Can buy token with bonding curve", async () => {
            ...
            const signature = await getTradeSignature(alice.address)
        ...
        it("Cannt buy token by send ETH to token contract", async () => {
            const buyFund = parseAmount(0.2)
            const feeInfo = await getBuyFeeData(buyFund)
            const buyAmount = await pump.getBuyAmountByValue(0, feeInfo.buyFund);
        ...
        it("Can sell token with bonding curve", async () => {
            const signature = await getTradeSignature(alice.address)
            await token.connect(alice).buyToken(parseAmount(100000000), ethers.ZeroAddress, 0, signature, {
        ...
        it("Can trade with otherone", async () => {
            ...
            const signature = await getTradeSignature(alice.address)
            const buyAmount = await pump.getBuyAmountByValue(0, feeInfo.buyFund);
            await token.connect(alice).buyToken(buyAmount, ethers.ZeroAddress, 0, signature, {
                value: buyFund
            })
        ...
        it("The sellsman cant purchase token if he did not created ipshare", async () => {
            ...
            const buyAmount = await pump.getBuyAmountByValue(0, feeInfo.buyFund);
            const signature = await getTradeSignature(alice.address)
            await expect(token.connect(alice).buyToken(buyAmount, bob, 0, signature, {
                value: buyFund
        ...
        it('Cannt make lp before list', async () => {
            const signature = await getTradeSignature(owner.address)
            await token.buyToken(parseAmount(100000000), ethers.ZeroAddress, 0, signature, {
                value: parseAmount(1)
            })
